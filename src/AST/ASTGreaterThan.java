...
public class ASTGreaterThan implements ASTNode{
	...
	public ASTGreaterThan(ASTNode t1, ASTNode t2) {
		left = t1;
		right = t2;
	}
	...
	public String toString(){
		return "ASTGreaterThan("+this.left.toString()+", "+this.right.toString()+")";
	}
	...
	public void compile(ICodeBlock code, Environment<Address> env) throws IOException, IdentifierAlreadyExistsException, NullIdentifierException {
		...
		code.emit("if_icmpgt "+TL); //jumps to the True Label if left (2nd on stack) > right (top of stack)
		//False branch
		code.emit("iconst_0");
