...
public class ASTGreaterEquals implements ASTNode{
	private ASTNode left;
	private ASTNode right;
	public ASTGreaterEquals(ASTNode t1, ASTNode t2) {
		left = t1;
		right = t2;
	}
	...
	public String toString(){
		return "ASTGreaterEquals("+this.left.toString()+", "+this.right.toString()+")";
	}
	...
	public void compile(ICodeBlock code, Environment<Address> env) throws IOException, IdentifierAlreadyExistsException, NullIdentifierException {
		...
		code.emit("if_icmpge "+TL); //jumps to the True Label if left (2nd on stack) >= right (top of stack)
		//False branch
		code.emit("iconst_0");
